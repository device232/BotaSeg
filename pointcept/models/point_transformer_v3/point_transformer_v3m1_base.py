...
@MODELS.register_module(name="PT-v3m1")
class PointTransformerV3SemSeg(nn.Module):
    def __init__(self, num_classes=0, **kwargs):
        ...
        enc_channels = kwargs.get("enc_channels", (32, 64, 128, 256, 512))
        out_channels = enc_channels[-1] if kwargs.get("cls_mode", False) else dec_channels[0]
        self.seg_head = nn.Linear(out_channels, num_classes) if num_classes > 0 else nn.Identity()
